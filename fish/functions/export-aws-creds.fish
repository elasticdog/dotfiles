function export-aws-creds --description 'Export AWS credentials into the shell environment'
	...
	if set -q $argv
		...
		if test -f $AWS_CREDENTIALS_FILE
			set -x AWS_ACCESS_KEY (ini-section $argv[1] $AWS_CREDENTIALS_FILE | awk '/^aws_access_key_id/{ print $3 }')
			set -x AWS_ACCESS_KEY_ID $AWS_ACCESS_KEY
			set -x AWS_SECRET_KEY (ini-section $argv[1] $AWS_CREDENTIALS_FILE | awk '/^aws_secret_access_key/{ print $3 }')
